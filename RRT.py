...
if __name__ == "__main__":
    ...
    import path_planner
    print("创建测试环境...") 
    env = OpenPitMineEnv(width=100, height=100)  # 创建一个较小的测试环境

    # 添加障碍物
    print("添加障碍物...")
    obstacles = [
        (20, 20, 30, 10),  # (x, y, width, height)
        (60, 50, 20, 20),
        (30, 70, 40, 10)
    ]
    ...
    for obs in obstacles:
        x, y, w, h = obs
        env.add_obstacle(x, y, w, h)

    print("添加关键点...")
    # 添加装载点和卸载点
    loading_point_idx = env.add_loading_point((20, 40))
    unloading_point_idx = env.add_unloading_point((70, 85))

    if loading_point_idx >= 0 and unloading_point_idx >= 0:
        print(f"装载点添加成功，索引: {loading_point_idx}")
        print(f"卸载点添加成功，索引: {unloading_point_idx}")
    else:
        print("关键点添加失败，请检查坐标是否有效")

    print("初始化优化RRT规划器...") 
    try:
        from path_planner import OptimizedRRTPlanner
        ...
        if path:
            ...
            print(f"  规划耗时: {end_time - start_time:.3f}s")
        ...
        print("无法导入OptimizedRRTPlanner，请确保path_planner.py文件存在")
